...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( recipe.getIngredients().size() < 1 ) {
            return new ResponseEntity( errorResponse( "Invalid number of ingrexients for " + recipe.getName() ),
     ...
     * Inventory, by making a DELETE request to the API endpoint and indicating
     * the recipe to delete (as a path variable) then saving the new recipe
     *
     * @param recipe
     *            The name of the new recipe
     * @return Success if the recipe could be saved; an error if the recipe does
     *         not exist
     */
    @PatchMapping ( BASE_PATH + "/recipes" )
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        ...
        return new ResponseEntity( errorResponse( "Error while creating recipe: " + recipe.getName() ),
                HttpStatus.BAD_REQUEST );
